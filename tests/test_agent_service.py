...
class ResponseFormatSupportTests(unittest.TestCase):
    ...
    def test_rewrite_google_images_navigation_actions_uses_direct_url_for_explicit_query(
        self,
    ) -> None:
        actions = [
    ...
    def test_rewrite_google_images_navigation_actions_uses_chrome_script_for_explicit_query(
        self,
    ) -> None:
        actions = [
            ActionModel.model_validate({"Click": {"position": [116, 116]}}),
        ...
        dumped = [action.model_dump(exclude_unset=True) for action in rewritten]
        self.assertIn(
            'tell application "Google Chrome"', dumped[0]["run_apple_script"]["script"]
        )
        self.assertIn(
    ...
    def test_rewrite_google_images_navigation_actions_can_derive_current_query(
        self,
    ) -> None:
        actions = [
    ...
    def test_rewrite_google_images_navigation_actions_can_derive_current_query_in_chrome(
        self,
    ) -> None:
        actions = [
            ActionModel.model_validate({"Hotkey": {"key": "ESC"}}),
    ...
    def test_rewrite_google_images_navigation_actions_replaces_done_only_when_goal_is_empty(
        self,
    ) -> None:
    ...
    def test_build_google_images_fallback_actions_uses_task_when_actor_response_is_empty(
        self,
    ) -> None:
        ...
        self.assertIn("run_apple_script", dumped[0])
        self.assertIn(
            "Tokyo%20Skytree%20%E6%99%B4%E7%A9%BA%E5%A1%94",
            dumped[0]["run_apple_script"]["script"],
        )
        self.assertEqual(dumped[1], {"wait": {}})

    def test_build_google_images_fallback_actions_prefers_chrome_when_task_names_it(
        self,
    ) -> None:
        fallback = build_google_images_fallback_actions(
    ...
    def test_build_google_images_fallback_actions_returns_done_for_stop_goal(
        self,
    ) -> None:
        fallback = build_google_images_fallback_actions(
            task="In Safari on the current Google search results page for 晴空塔, switch to the Google Images results page and stop only when image thumbnails are visible.",
