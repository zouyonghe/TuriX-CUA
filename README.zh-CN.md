...
想用你自己的模型？**先把 `config.example.json` 复制成 `config.json`，再修改即可。**
...
如果任务中断，想从上次位置继续，请在 `config.json` 中设置固定的 `agent_id` 并开启 `resume`：
```json
{
    "agent": {
         "resume": true,
